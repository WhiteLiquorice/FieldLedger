...
const PROJECT_ID = 'fieldledger-stg';
const API_KEY = 'AIzaSyA0D8wQ1MWDZQGvj_9qMM5qkUT5J264G_0';
const APP_ID = '1:814312453280:web:022f1aa8448b3d8df085c5';
const BUCKET_NAME = 'fieldledger-stg.firebasestorage.app';
const FUNCTIONS_HOST = 'https://us-central1-fieldledger-stg.cloudfunctions.net';
const STRIPE_PRICE_ID = 'price_1U80EbJRMfcFhxi8TJ9UJwAN';

console.log('====================================================');
console.log('  FieldLedger Live Staging Acceptance Suite');
...
async function getAppCheckToken() {
  const res = await fetch(`https://firebaseappcheck.googleapis.com/v1/projects/${PROJECT_ID}/apps/${APP_ID}:exchangeDebugToken?key=${API_KEY}`, {
    ...
    body: JSON.stringify({ debugToken: 'a1b2c3d4-e5f6-4a7b-8c9d-0e1f2a3b4c5d' })
  });
...
async function run() {
  ...
  try {
    ...
    const deletionRes = await invokeCallable('requestWorkspaceDeletion', {
      ...
      reason: 'Automated staging acceptance suite verification.'
    }, ownerA.idToken, appCheckToken);
    ...
    record('Acceptance Suite Execution', 'FAILED', error.stack || error.message);
    console.error('\nAcceptance check aborted with error:\n', error);
  } finally {
    console.log('\n--- Cleaning up temporary test identities ---');
  ...
  console.log('====================================================');
  console.log(`  Acceptance Summary: ${passedCount} PASSED, ${failedCount} FAILED`);
  console.log('====================================================\n');
